...
class SendClinicAccreditationJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $taxonomy = Taxonomy::where('name', 'Clinic Accreditation')->first();
            if (!empty($taxonomy?->email_recipients)) {
                $recipients = array_filter((array) $taxonomy->email_recipients);
                if (!empty($recipients)) {
                    Mail::to($recipients)->queue(new UserClinicAccreditationMail($this->data, $this->remotePath)); 
                }
            }
    }
}
